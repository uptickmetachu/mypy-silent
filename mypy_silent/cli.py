...
from mypy_silent.parser import (
    UNUSED_IGNORE_MESSAGES,
    FilePosition,
    get_info_from_mypy_output,
)
...
def mypy_silent(
    ...
    nofmt: bool = typer.Option(
        False, help="append a fmt: skip to each line to skip formatting"
    ),
) -> None:
    ...
    for info in infos:
        ...
        if info.message in UNUSED_IGNORE_MESSAGES:
            ...
            new_content = add_type_ignore_comment(
                old_content, error_code=info.error_code, nofmt=nofmt
            )
